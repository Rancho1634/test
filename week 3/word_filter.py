...
def word_filter_counter(text, filter_words):
    ...
    word_counts = {word: words.count(word) for word in filter_words}
    print(word_counts)

    #filter_words = text.lower().split()
    #count = text.count(filter_words)
    #print(filter_words, count)
   # thisdict = dict.fromkeys(filter_words, count)
    #print(thisdict)
